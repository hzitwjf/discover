      ...
      <dd style="margin-left: 100px">
        <c:forEach items="${requestScope.answerList}" var="list">
          <c:forEach items="${list}" var="a">
            <c:if test="${p.proId==a.proId and a.proId!=35}">
            <span>
            ...
            <c:if test="${a.proId==35 and p.proId==35}">
                  <textarea name="" id="textarea" cols="30" rows="10" proId="${a.proId}" proContent="${p.proContent}" ansSc="此题无法评分，就是这样" ></textarea>
            </c:if>
          </c:forEach>
        </c:forEach>
      </dd>
